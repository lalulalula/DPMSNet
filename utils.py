...
def random_crop(img, mask, patch_size, pos_prob=None): 
    # 随即裁剪出一个patch_size大小的图片，可以选择优先裁剪带目标的图片
    h, w = img.shape
...
def get_optimizer(net, optimizer_name, scheduler_name, optimizer_settings, scheduler_settings):
    ...
    # if optimizer_name == 'Adam':
    #     optimizer = torch.optim.Adam(net.parameters(), lr=optimizer_settings['lr'], weight_decay=optimizer_settings.get('weight_decay', 0))
    elif optimizer_name == 'AdamW':
        optimizer = torch.optim.AdamW(net.parameters(), lr=optimizer_settings['lr'], weight_decay=optimizer_settings.get('weight_decay', 0))
